...
if (isset($_FILES['file']) && !empty($_FILES['file']['tmp_name'])) {
    ...
    if ($type == 'csv') {
        ...
        $data = json_decode(json_encode($xml), true)['event'];
    } elseif ($type == 'txt') {
        $file = fopen($_FILES['file']['tmp_name'], 'r');
        while ($row = fgetcsv($file)) {
            $data[] = $row;
    ...
    if (isset($data) && !empty($data)) {    
        ...
        foreach ($data as $k => $row) {
            ...
            if (isset($row['datestart'])) {
                $row['datestart'] = date('Y-m-d H:i', strtotime(str_replace('/','-', $row['datestart'])));
            }
            if (isset($row['dateend'])) {
                $row['dateend'] = date('Y-m-d H:i', strtotime(str_replace('/','-', $row['dateend'])));
            }
            ...
            $stmt = $pdo->prepare('INSERT IGNORE INTO events VALUES (' . $values . ')');
            $stmt->execute(array_values($row));
            $i += $stmt->rowCount();
        }
        header('Location: events.php?success_msg=4&imported=' . $i);
        exit;
    }
}
?>
<?=template_admin_header('Import Events', 'events', 'import')?>

<form action="" method="post" enctype="multipart/form-data">

    <div class="content-title responsive-flex-wrap responsive-pad-bot-3">
        <h2 class="responsive-width-100">Import Events</h2>
