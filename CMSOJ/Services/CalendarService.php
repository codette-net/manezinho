...
class CalendarService
{
    ...
    public function getEvents(int $uid, int $year, int $month): array
    {
        ...
        return $this->addRecurringEvents($events, $year, $month);
    }
    ...
    private function expandDaily(array $event, int $year, int $month): array
    {
        ...
        // Generate up to 31 extra days (enough for UI)
        for ($i = 1; $i <= 31; $i++) {
            $start->modify('+1 day');
            $end->modify('+1 day');
            ...
            // Only keep occurrences that touch this month
            if (date('Y-m', strtotime($clone['datestart'])) === sprintf('%04d-%02d', $year, $month) ||
                date('Y-m', strtotime($clone['dateend']))   === sprintf('%04d-%02d', $year, $month)
            ) {
                $out[] = $clone;
            }
        }
    ...
    private function expandWeekly(array $event, int $year, int $month): array
    {
        ...
        // Go a few weeks back and forward
        for ($i = -4; $i <= 8; $i++) {
            ...
            if (date('Y-m', $startClone->getTimestamp()) === $monthStr ||
                date('Y-m', $endClone->getTimestamp())   === $monthStr
            ) {
                $clone              = $event;
    ...
    private function expandYearly(array $event, int $year, int $month): array
    {
        ...
        if ((int)$start->format('m') === $month || (int)$end->format('m') === $month) {
            $clone              = $event;
            $clone['datestart'] = $start->format('Y-m-d H:i:s');
            $clone['dateend']   = $end->format('Y-m-d H:i:s');
            $out[]              = $clone;
    ...
    public function attachEventsToDays(array $days, array $events): array
    {
        foreach ($events as $event) {
            ...
            foreach ($days as &$day) {
                ...
                if ($dayDate >= (clone $start)->setTime(0, 0, 0) &&
